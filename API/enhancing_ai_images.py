...
HEADERS = {"Authorication": f"Bearer {HF_API_KEY}", "Accept": "image/png"}
...
def generate_image_from_text(prompt):
    ...
    for model in MODELS:
        ...
        for _ in range(3):
            ...
            if r.status_code == 503 and "application/json" in ct:
                try:
                    wait_s = int(r.json().get("estimated-time", 5))
                except Exception:
                    wait_s = 5
                time.sleep(wait_s + 1)
                continue
    ...
    def main():
        ...
        print("this progream generates an image from text and applesi post-processing effects.")
        ...
        while True:
            ...
            try:
                ...
                processed_image = post_process_iamge(image)
