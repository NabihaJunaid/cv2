...
hands = Hands.Hands(min_detection_confidence=0.7, min_tracking_confidence=0.5)
...
while True:
  ...
  if res.multi_hand_landmarks and res.multi_handedness:
    for i, hand in enumerate(res.multi_hand_landmarks):
      ...
      dist = float(np.hypot(ip[0]-tp[0], ip[1]))
      ...
      if label == "Left":
        ...
        cv2.putText(img, f"{pct}%", (40, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (255, 0, 0), 3)
        ...
        cv2.putText(img, f"{b}%", (w-110, 450), cv2.FONT_HERSHEY_COMPLEX, 1, (0, 255, 0), 3)
